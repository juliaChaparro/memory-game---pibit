...
exports.setupAuthRoutes = exports.authRouter = void 0;
...
let prisma;
const setupAuthRoutes = (prismaClient) => {
    prisma = prismaClient;
...
exports.authRouter.post('/google', async (req, res) => {
    try {
        ...
        if (!token) {
            logger_1.logger.warn('[AUTH_FAIL] Token não fornecido');
        ...
        if (!payload) {
            logger_1.logger.warn('[AUTH_FAIL] Payload do token inválido');
        ...
        if (!user) {
            ...
            if (user) {
                ...
                logger_1.logger.info(`[AUTH_SUCCESS] Conta existente vinculada ao Google: ${email}`);
            }
            else {
                ...
                logger_1.logger.info(`[AUTH_SUCCESS] Nova conta Google registrada: ${email}`);
            }
        }
        else {
            ...
            logger_1.logger.info(`[AUTH_SUCCESS] Usuário Google logado: ${email}`);
        }
        // Geração do JWT
        const sessionToken = jsonwebtoken_1.default.sign({ userId: user.id, email: user.email, name: user.name }, JWT_SECRET, { expiresIn: '7d' });
